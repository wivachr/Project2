...
- **Team-member (`manipulator`) edits happen in 4 places** — `project/addmanipulator.php` (student), `addmanipulator2.php` (officer), `editmanipulator2.php` (edit, both portals), and `regis/registerproject.php` (initial). Guarding only the "add" paths is bypassable through "edit". All four enforce: student exists in `student`, not already in this project, not in another project active this term (ignoring status 0/18); the edit path excludes its own row from the dup check. `regis/check_ajax/canuse.php` has the same "already this term" test but runs only in client JS, so `registerproject.php` re-checks server-side inside its `GET_LOCK` (a double-clicked submit created duplicate registrations otherwise). `project/editmanipulator.php` (no "2") was dead code (INSERT column-count mismatch) — deleted
