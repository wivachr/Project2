...
### Team-Member (`manipulator`) Integrity Lives in Four Places — Guard Them Together
Who is in a project group is edited from more paths than it looks: `project/addmanipulator.php` (student adds), `project/addmanipulator2.php` (officer adds), `project/editmanipulator2.php` (both portals edit an existing member), and the initial `regis/registerproject.php`. A guard on only the "add" paths is bypassable via the "edit" path (add a valid member, then edit its `id_student` to anything). All four now enforce the same three rules — the student must exist in `student`, must not already be in this project, and must not be in another project that is still active this academic year/term (ignoring statuses 0 and 18/cancelled) — and the edit path excludes the row being edited from its own duplicate check so a phone-only edit still works. The "already registered this term" rule is also implemented in `regis/check_ajax/canuse.php`, but that only runs from client-side JS on blur; `registerproject.php` re-does it server-side inside its `GET_LOCK` section, because nothing else validated a double-clicked/replayed submit (which is how duplicate registrations of the same team, e.g. 691019/691020, were created). Note `project/editmanipulator.php` (no "2") was dead code whose INSERT never matched the column count — deleted; the working edit path is `editmanipulator2.php`.
