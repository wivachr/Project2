...
When a feature "works locally but silently does nothing on the server", suspect strict mode first: grep the handler for `''` (or `'0'`) being written into a `date`/`int` column, and reproduce by running `SET GLOBAL sql_mode='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';` locally before testing (restore it afterward). Note `exam/showexam.php` still treats `0000-00-00` as the "ยังไม่ได้ยื่นเรื่อง" sentinel for legacy rows — don't "clean up" that check without migrating the old data. See `UBUNTU_MIGRATION.md` §0.1.
