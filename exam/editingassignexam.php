...
OR committee.id_teacher = '$t[2]'
	...
	?>
		...
		var str = Math.random();
		var  qstr ="exam/assigningexam2.php?pop="+str;
...
<table width="100%" border="0">
  <tr>
    <td colspan="2" align="center"><h3>แก้ไขการจัดสอบโครงงานพิเศษ รหัส 
    <?=$idproject?></h3>
              <?  
include('../connectdatabase.php'); 
	  $num = 0;
...
<p>
  ...
  <tr>
    <td width="521" align="right">วันที่จัดสอบ :</td>
    <td width="467" align="left">
    	  		<input name="dateassex" type="text" id="dateassex" value="<?=$date?>" size="9" maxlength="9" readonly="readonly"><a href="javascript:NewCal('dateassex','ddmmyyyy')"><img src="image/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
  </tr>
  <tr>
    <td align="right">เวลาที่จัดสอบ :</td>
    <td align="left">
      <select name="timeassex" id="timeassex">
		...
		for($i=8;$i<21;$i++)
		{
			...
			$i="0".$i;
		?>
        <option <? if($i.":00"==$time)echo "selected='selected'"; ?>value="<?=$i?>:00"><?=$i?>:00</option>
        <?
		}
		?>
    </select></td>
  ...
  <tr>
    ...
    <td align="left">
      <select name="endtimeassex" id="endtimeassex">
		...
		for($i=8;$i<21;$i++)
		{
			...
			$i="0".$i;
		?>
        <option <? if($i.":00"==$endtime)echo "selected='selected'"; ?>value="<?=$i?>:00"><?=$i?>:00</option>
        <?
		}
		?>
    </select></td>
    ...
    <td align="left">
          ...
          <option value="0">---เลือกห้องเรียน---</option>
			  ...
			  {
				  ?>
                  <option value="<?=$rs[0]?>" <? if($rs[0]==$room){?>selected="selected"<? }?>>
                    <? echo $rs[1]; ?>
                  </option>
                  <?
			  }
  ...
  <tr>
    <td colspan="2" align="center"><input type="button" name="assigexb" id="assigexb" value="จัดตารางสอบ" onclick="editingassignex(<?=$id?>)" />
    <input type="submit" name="cancel" id="cancel" onclick="cancelea();" value="ยกเลิก" /></td>
  </tr>
</table>
