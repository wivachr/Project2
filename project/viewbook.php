...
<h2>
 ...
 $sql = "select * from committee,teacher,academictitle where teacher.id_academictitle = academictitle.id_academictitle AND committee.id_teacher=teacher.id_teacher AND id_project='".$rs[0]."' AND position='ที่ปรึกษา'";
	  ...
	  ?>
ส่งปริญญานิพนธ์ฉบับสมบูรณ์และCD โครงงานพิเศษ รหัส
<?=$idview?>
</h2>
...
<p>
  <input type="button" name="button" id="button" value="ยืนยันการส่ง" onclick="book('<?=$idview?>')" />
  <input type="button" name="button2" id="button2" value="ยกเลิก" onclick="cancelbook()" />
</p>
<? 			 
   			}
