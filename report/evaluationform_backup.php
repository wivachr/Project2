...
<? include('../connectdatabase.php'); 
	  ...
	  $coad="";
			  ...
			  while($rs2 = mysqli_fetch_array($result))
			  {
			   ...
			   $sql = "select * from committee,teacher,academictitle where id_project='".$rs[0]."' AND position='?????????' AND committee.id_teacher=teacher.id_teacher AND teacher.id_academictitle=academictitle.id_academictitle";
			  $result = mysqli_query($connect, $sql);
			  ...
			  while($rs2 = mysqli_fetch_array($result))
			  {
			   ...
			   $sql = "select * from committee,teacher,academictitle where academictitle.id_academictitle = teacher.id_academictitle AND committee.id_teacher=teacher.id_teacher AND id_project='".$rs[0]."' AND position='??????'";
			  $result = mysqli_query($connect, $sql);
			  while($rs3 = mysqli_fetch_array($result))
			  {
				  $master = $rs3[17].$rs3[7];
...
<div id="apDiv2">
  ...
  <table width="100%" border="0">
    <tr>
      <td colspan="5" valign="top"><strong>???????????????????????????????? </strong></td>
    </tr>
    <?
	for($i=0;$i<$num;$i++)
	{
  ...
  <table width="100%" border="0">
    <tr>
      <td width="26%" align="left"> <strong>????????????????</strong></td>
  ...
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <td width="9%" valign="middle"><strong>????</strong></td>
